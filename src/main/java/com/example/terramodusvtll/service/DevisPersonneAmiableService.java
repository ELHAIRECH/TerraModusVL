package com.example.terramodusvtll.service;

import com.example.terramodusvtll.entities.Immob;
import com.example.terramodusvtll.entities.TypeBien;
import com.example.terramodusvtll.entities.devisPersonne.DevisPersonneAmiable;
import com.example.terramodusvtll.repositories.DemandeurPersonneRepository;
import com.example.terramodusvtll.repositories.ImmobRepository;
import com.example.terramodusvtll.repositories.TypeDevisRepository;
import com.example.terramodusvtll.repositories.devisPersonne.DevisPersonneAmiableRepository;
...
import static com.example.terramodusvtll.entities.TypeBien.*;

@Service
public class DevisPersonneAmiableService {

    @Autowired
    private DevisPersonneAmiableRepository deviseRepository;
    @Autowired
    ...
    public DevisPersonneAmiable saveDevise(DevisPersonneAmiable devise) throws MessagingException, jakarta.mail.MessagingException, JRException, IOException, ParseException, XmlException {
        ...
        String[] headerPlaceholders = new String[]{"NOM", "PHONE", "TID", "VAL","IDENTM"};
        ...
        ind = 0;
        for (Immob immob : devise.getImmobs()) {
            List<TypeBien> list = Arrays.asList(Mais,Villa,Infra1,Infra2,Infra3,Infra4,Infra5,Infra6,Infra7);
            if (list.contains(immob.getTypeBien()) && immob.getQantite()== 1) bien += "une";
            else bien += converters.asWords(immob.getQantite());
            bien += " "+immob.getTypeBien().getExactName();
        ...
        String[] headerPlaceholdersVal = new String[]{devise.getDemandeurPersonne().getNom() + " " + devise.getDemandeurPersonne().getPrenom(), String.valueOf(devise.getDemandeurPersonne().getPhone())
                ,tid ,iValue, devise.getDemandeurPersonne().getEmail()};
        ...
        FileInputStream inputStream = new FileInputStream("src/main/resources/AmiablePers.docx");
                            ...
                            for (XWPFRun run2 : paragraph2.getRuns()) {
                                ...
                                if (runText2.contains("CP"+index)) {
                                    ...
                                    if (!consDocsCel5Cop1[0].equals("--"))
                                        HT += Double.valueOf(consDocsCel5Cop1[0]);
                                    run2.setText(runText2, 0);
                                }
                            }
                        }
        ...
        DevisPersonneAmiable savedDevis = deviseRepository.save(devise);
        ...
        public int update(DevisPersonneAmiable devise){
            DevisPersonneAmiable devise1 = deviseRepository.getOne(devise.getId());
